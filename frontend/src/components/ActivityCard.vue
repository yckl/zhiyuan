...
const isDeadlinePassed = (activity: any) => {
  const deadlineRaw = activity?.registerEnd || activity?.deadline
...
const getDisplayStatusName = (activity: any) => {
  if (activity?.status === 2 && isDeadlinePassed(activity)) return '已截止'
  return activity?.statusName || '未知状态'
}
...
const getButtonText = (activity: any) => {
  if (activity.status === 2) {
    if (isDeadlinePassed(activity)) return '报名已截止'
