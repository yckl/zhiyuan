        ...
        <el-table-column label="封面" width="100" align="center">
          <template #default="scope">
            <el-image
              ...
              :src="getCoverImage(scope.row.coverImage)"
              fit="cover"
              :preview-src-list="getPreviewList(scope.row.coverImage)"
              preview-teleported>
  <template #error>
    <img :src="'/default-cover.jpg'" style="width:100%;height:100%;object-fit:cover"/>
      ...
      <div class="hidden-md-and-up mobile-card-list">
        <div v-for="item in activityList" :key="item.id" class="mobile-card">
           ...
           <div class="card-body">
              <el-image :src="getCoverImage(item.coverImage)" class="card-cover" fit="cover">
  <template #error>
    <img :src="'/default-cover.jpg'" style="width:100%;height:100%;object-fit:cover"/>
  </template>
</el-image>
...
const getCoverImage = (url?: string | null) => {
  const resolved = request.resolveUrl(url)
...
const fetchData = async () => {
  loading.value = true
  try {
    const res = await request.get('/admin/activity/list', { ...queryParams })
