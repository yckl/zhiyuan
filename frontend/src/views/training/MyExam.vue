          ...
          <div v-for="row in filteredRecords" :key="row.id" class="mobile-card" @click="openDetail(row)">
            <div class="card-header-mobile">
              <div class="title-section" @click.stop="goToCourse(row.courseId)">
                <span class="course-title">{{ getDisplayCourseName(row) }}</span>
...
const getDisplayCourseName = (record: ExamRecord) => {
  return record.courseName || record.courseTitle || `课程${record.courseId}`
}
...
const filteredRecords = computed(() => {
  ...
  if (searchKeyword.value) {
    ...
    list = list.filter(r => getDisplayCourseName(r).toLowerCase().includes(kw))
  }
...
const fetchRecords = async () => {
  ...
  try {
    ...
    if (res.code === 200 && res.data?.length) {
      examRecords.value = res.data.map((r: any) => ({
        ...r,
        courseName: r.courseName || r.courseTitle || '',
        passed: r.passed === true || r.passed === 1,
        createTime: r.createTime || r.submitTime || ''
      }))
    } else {
...
.records-card {
  ...
  .action-cell {
    ...
    .retry-btn {
      background: linear-gradient(135deg, #67c23a, #52b42b);
      color: #fff;
      box-shadow: 0 6px 14px rgba(103, 194, 58, 0.25);

      &:hover {
        transform: translateY(-1px);
        box-shadow: 0 8px 18px rgba(103, 194, 58, 0.33);
      }
    }
  }
}
