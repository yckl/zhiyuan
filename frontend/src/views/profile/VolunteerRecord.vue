      ...
      <div v-loading="loading" class="timeline-wrapper">
        <el-timeline v-if="finishedList.length > 0">
          <el-timeline-item
            ...
            :key="item.id"
            :timestamp="formatDate(item.signOutTime || item.activityEndTime || item.signInTime || item.createTime)"
            placement="top"
            color="#0093E9"
          ...
          >
            <div class="record-card shadow-hover">
              <div class="card-header">
                <h3 class="article-title">{{ getDisplayTitle(item) }}</h3>
              ...
              <div class="card-body">
                <div class="info-row">
                  ...
                  <span>服务地点：{{ item.location || '校内指定区域' }}</span>
                </div>
                <div class="info-row">
                  <span>组织单位：{{ item.organizerName || '青年志愿者协会' }}</span>
                </div>
        ...
        <el-empty v-else-if="!loading" description="尚无已签到或已完成的志愿记录，坚持就是胜利！" :image-size="120" />
      </div>
    </div>
  </div>
</template>
...
const getDisplayHours = (item: any) => Number(item.actualHours ?? item.serviceHours ?? 0)
...
const getRecordTagText = (item: any) => {
  const hours = getDisplayHours(item)
  if (item.status === 3) return hours > 0 ? `已完成 +${hours} 小时` : '已完成'
  if (item.status === 2) return hours > 0 ? `已签到 ${hours} 小时` : '已签到（待录入工时）'
  return '状态未知'
}

const getDisplayTitle = (item: any) => {
  const candidates = [item.activityTitle, item.title, item.activityName]
  const name = candidates.find((v: any) => typeof v === 'string' && v.trim() && !/^\d+$/.test(v.trim()))
  return name || `活动 #${item.activityId || item.id || '-'}`
}

const fetchRecords = async () => {
  loading.value = true
  try {
    const res = await request.get('/registration/my', { page: 1, size: 100 })
    if (res.code === 200) {
      finishedList.value = (res.data?.records || []).filter((item: any) => item.status === 2 || item.status === 3)
      // 按时间倒序
      finishedList.value.sort((a, b) => {
        const t1 = a.signOutTime || a.activityEndTime || a.signInTime || a.createTime
        const t2 = b.signOutTime || b.activityEndTime || b.signInTime || b.createTime
