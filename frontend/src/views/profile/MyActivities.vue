      ...
      <div v-else class="hidden-sm-and-down">
        ...
        >
          ...
          <el-table-column label="报名时间" width="150">
            <template #default="{ row }">
              <div class="time-cell">
                <div class="date-part">{{ formatDateOnly(row.createTime) }}</div>
                <div class="time-part">{{ formatTimeOnly(row.createTime) }}</div>
              </div>
            </template>
          </el-table-column>
            ...
            <template #default="{ row }">
              <div class="action-buttons">
                <el-button
                  ...
                  @click.stop="openCheckin(row)"
                >
                  <el-icon><Location /></el-icon> 签到
                </el-button>
                <el-button type="primary" size="small" class="action-btn detail-btn" @click.stop="openDrawer(row)">查看详情</el-button>
                <el-button v-if="canCancel(row)" type="danger" size="small" class="action-btn cancel-btn" @click.stop="promptCancel(row)">取消报名</el-button>
              </div>
            </template>
          </el-table-column>
        </el-table>
      ...
      <div class="hidden-md-and-up mobile-list">
        <div v-for="row in filteredRegistrations" :key="row.id" class="mobile-card" @click="openDrawer(row)">
          <div class="mobile-card-head">
            <span class="mobile-title">{{ getDisplayTitle(row) }}</span>
      ...
      <div class="drawer-content" v-if="currentRow">
        ...
        <div class="drawer-banner">
          ...
          <div class="banner-content">
            <h2 class="banner-title">{{ getDisplayTitle(currentRow) }}</h2>
...
const updateSummary = (totalCount?: number) => {
  const all = registrations.value
  animateNumber(totalCount ?? all.length, v => { animTotal.value = v })
...
const getDisplayTitle = (row: any) => {
  const candidates = [row?.activityTitle, row?.title, row?.activityName]
  const name = candidates.find((v: any) => typeof v === 'string' && v.trim() && !/^\d+$/.test(v.trim()))
  return name || `活动 #${row?.activityId || row?.id || '-'}`
}
...
.detail-btn {
  ...
  white-space: nowrap;
  box-shadow: 0 6px 14px rgba(5, 213, 179, 0.28);
...
.cancel-btn {
  box-shadow: 0 6px 14px rgba(245, 108, 108, 0.22);
}
