    ...
    <img :src="'/default-cover.jpg'" style="width:100%;height:100%;object-fit:cover"/>
          ...
          <div class="item-action-area">
            <template v-if="detailItem.status === 0">
              <!-- 如果是功能卡 -->
              <template v-if="isCardType(detailItem)">
...
const isCardType = (item: BackpackItem | string) => {
  const type = typeof item === 'string' ? item : (item.propType || '')
  const name = typeof item === 'string' ? '' : (item.propName || '')
  const content = typeof item === 'string' ? '' : (item.propContent || '')
  const text = `${type}|${name}|${content}`.toUpperCase()
  return text.includes('MAKEUP_CARD') ||
    text.includes('SKIP_REVIEW_CARD') ||
    text.includes('CARD') ||
    text.includes('补签') ||
    text.includes('免审核')
}
...
const fetchBackpack = async () => {
  ...
  try {
    ...
    if (res.code === 200) {
      backpackItems.value = (res.data || []).filter((item: BackpackItem) => (item.quantity || 0) > 0)
    }
  } catch (error) {
