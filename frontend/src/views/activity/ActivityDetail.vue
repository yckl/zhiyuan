...
const isPassed = (dateLike: any) => {
  if (!dateLike) return false
  const t = dayjs(dateLike)
  if (!t.isValid()) return false
  return t.isBefore(dayjs())
}

const isFuture = (dateLike: any) => {
  if (!dateLike) return false
  const t = dayjs(dateLike)
  if (!t.isValid()) return false
  return t.isAfter(dayjs())
}
...
const canRegister = computed(() => {
  ...
  if (isFuture(regStart)) return false

  const deadline = activity.value.registerEnd || activity.value.deadline
  if (isPassed(deadline)) return false
...
const registerBtnText = computed(() => {
  ...
  // 检查报名开启时间
  const regStart = activity.value.registerStart
  if (isFuture(regStart)) return '报名尚未开始'
