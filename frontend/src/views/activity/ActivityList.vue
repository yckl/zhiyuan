  ...
  <div class="activity-list-page" :class="{ 'is-mobile': isMobile }">
    ...
    <template v-if="isMobile">
      ...
      <div class="mobile-feed" ref="feedRef" @scroll="handleScroll">
        <div v-if="listUpdating && activities.length > 0" class="list-updating-mask">
          <el-icon class="is-loading"><Loading /></el-icon>
          <span>{{ listUpdatingText }}</span>
        </div>
        ...
        <!-- 卡片信息 -->
        <div v-else class="mobile-list-static" :class="{ 'is-updating': listUpdating }">
    ...
    <template v-else>
      ...
      <div class="pc-activity-grid">
        <div v-if="listUpdating && activities.length > 0" class="list-updating-mask pc">
          <el-icon class="is-loading"><Loading /></el-icon>
          <span>{{ listUpdatingText }}</span>
        </div>
        ...
        <!-- 卡片网格 -->
        <div v-else class="pc-grid-inner" :class="{ 'is-updating': listUpdating }">
...
const FILTER_STATE_KEY = 'activityListState'
...
const listUpdating = ref(false)
const listUpdatingText = ref('正在更新结果...')
...
const restoreListState = () => {
  ...
  try {
    ...
    if (typeof saved.page === 'number' && saved.page > 0) queryParams.page = saved.page
    if (typeof saved.size === 'number' && saved.size > 0) queryParams.size = saved.size
    if (typeof saved.title === 'string') queryParams.title = saved.title
    queryParams.categoryId = typeof saved.categoryId === 'number' ? saved.categoryId : null
    queryParams.organizerId = typeof saved.organizerId === 'number' ? saved.organizerId : null
    queryParams.status = typeof saved.status === 'number' ? saved.status : null
    if (typeof saved.orderBy === 'string' && saved.orderBy) queryParams.orderBy = saved.orderBy
  } catch (e) {
    console.warn('恢复活动筛选状态失败:', e)
  }
}
...
const fetchActivities = async () => {
  const useOverlay = activities.value.length > 0
  if (useOverlay) {
    listUpdating.value = true
    listUpdatingText.value = '正在更新结果...'
  } else {
    loading.value = true
  }
  try {
    ...
    // 前后端状态口径兜底统一：筛选“报名中”时，剔除已过报名截止时间的数据
    if (queryParams.status === 2) {
      records = records.filter((item: any) => {
        const deadline = item.registerEnd || item.deadline
...
const handleRecommend = async () => {
  ...
  if (activities.value.length > 0) {
    listUpdating.value = true
    listUpdatingText.value = '正在为您匹配活动...'
  }
  try {
