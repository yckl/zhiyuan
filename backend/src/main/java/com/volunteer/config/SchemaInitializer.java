...
public class SchemaInitializer implements CommandLineRunner {
    ...
    private void updateExpiredNoticeDates() {
        try {
            ...
            if (!hasColumn("notice", "end_time")) {
                log.info("Notice table has no 'end_time' column, skipping notice date migration.");
                return;
            }
            ...
            Integer expiredCount = jdbcTemplate.queryForObject(countSql, Integer.class);
            if (expiredCount != null && expiredCount > 0) {
                String startColumn = hasColumn("notice", "start_time") ? "start_time" : "create_time";
                String offsetSql = "SELECT DATEDIFF(NOW(), MIN(COALESCE(" + startColumn + ", create_time))) + 7 " +
                        "FROM notice WHERE end_time IS NOT NULL AND end_time < NOW() AND is_deleted = 0";
                Integer offsetDays = jdbcTemplate.queryForObject(offsetSql, Integer.class);
                if (offsetDays != null && offsetDays > 0) {
                    String updateSql;
                    int updated;
                    if (hasColumn("notice", "start_time")) {
                        updateSql = "UPDATE notice SET " +
                        ...
                        updated = jdbcTemplate.update(updateSql, offsetDays, offsetDays);
                    } else {
                        updateSql = "UPDATE notice SET " +
                                "end_time = CASE WHEN end_time IS NOT NULL THEN DATE_ADD(end_time, INTERVAL ? DAY) ELSE NULL END " +
                                "WHERE end_time IS NOT NULL AND end_time < NOW() AND is_deleted = 0";
                        updated = jdbcTemplate.update(updateSql, offsetDays);
                    }
    ...
    /**
     * 清理数据库中指向不存在本地文件的头像引用，避免反复触发 404 日志
     */
    private void cleanMissingLocalAvatarRefs() {
        try {
            String checkTable = "SELECT count(*) FROM information_schema.tables WHERE table_name = 'sys_user' AND table_schema = DATABASE()";
            Integer tableExists = jdbcTemplate.queryForObject(checkTable, Integer.class);
            if (tableExists == null || tableExists == 0) {
                return;
            }
            ...
            List<Map<String, Object>> users = jdbcTemplate.queryForList(
                    "SELECT id, username, avatar FROM sys_user WHERE avatar LIKE '/api/files/%'");
            int cleaned = 0;

            for (Map<String, Object> user : users) {
                Object avatarObj = user.get("avatar");
                if (!(avatarObj instanceof String avatar) || avatar.isBlank()) {
                    continue;
                }

                String relativePath = avatar.replaceFirst("^/api/files/", "");
                Path avatarPath = uploadRoot.resolve(relativePath).normalize();
                if (!avatarPath.startsWith(uploadRoot)) {
                    log.warn("Skip suspicious avatar path for user {}: {}", user.get("username"), avatar);
                    continue;
                }

                if (!Files.exists(avatarPath)) {
                    jdbcTemplate.update("UPDATE sys_user SET avatar = NULL WHERE id = ?", user.get("id"));
                    cleaned++;
                    log.info("Cleared missing local avatar for user {}: {}", user.get("username"), avatar);
                }
            }

            if (cleaned > 0) {
                log.info("Cleaned {} missing local avatar references.", cleaned);
            }
        } catch (Exception e) {
            log.warn("Failed to clean missing local avatar references: {}", e.getMessage());
        }
    }
