...
public class ActivityServiceImpl implements ActivityService {
    ...
    public Page<ActivityDTO> pageActivities(ActivityPageQuery query) {
        ...
        if (query.getStatus() != null) {
            ...
            // 当筛选"报名中"(status=2)时，严格与详情页 canRegister 逻辑一致：
            // 1) registerStart 为空或 <= now
            // 2) deadline 取 registerEnd 优先，否则取 deadline；且该值为空或 >= now
            if (query.getStatus() == 2) {
                LocalDateTime now = LocalDateTime.now();
                queryWrapper.and(w -> w
                    .isNull(Activity::getRegisterStart)
                    .or()
                    .le(Activity::getRegisterStart, now));
                queryWrapper.and(w -> w
                    .and(x -> x
                        .isNotNull(Activity::getRegisterEnd)
                        .ge(Activity::getRegisterEnd, now))
                    .or(x -> x
                        .isNull(Activity::getRegisterEnd)
                        .and(y -> y
                            .isNull(Activity::getDeadline)
                            .or()
                            .ge(Activity::getDeadline, now))));
            }
        }
