...
public class VolunteerServiceImpl implements VolunteerService {
    ...
    public VolunteerDTO getVolunteerProfile(Long userId) {
        ...
        List<ActivityRegistration> effectiveRegistrations = registrationMapper.selectList(statsQuery);

        Map<Long, Activity> activityMap = Collections.emptyMap();
        if (!effectiveRegistrations.isEmpty()) {
            List<Long> activityIds = effectiveRegistrations.stream()
                    .map(ActivityRegistration::getActivityId)
                    .filter(Objects::nonNull)
                    .distinct()
                    .toList();
            activityMap = activityMapper.selectBatchIds(activityIds).stream()
                    .collect(Collectors.toMap(Activity::getId, a -> a));
        }
        final Map<Long, Activity> finalActivityMap = activityMap;

        // 1. 服务次数统计（已签到和已完成都计入参与记录）
        int serviceCount = effectiveRegistrations.size();

        // 2. 时长统计：优先取结算后的实际工时，否则回退到活动配置工时，保证记录页与统计口径一致
        BigDecimal totalHours = effectiveRegistrations.stream()
                .map(r -> {
                    if (r.getActualHours() != null) {
                        return r.getActualHours();
                    }
                    Activity activity = finalActivityMap.get(r.getActivityId());
                    return activity != null && activity.getServiceHours() != null
                            ? activity.getServiceHours()
                            : BigDecimal.ZERO;
                })
                .reduce(BigDecimal.ZERO, BigDecimal::add);

        int totalPoints = effectiveRegistrations.stream()
    ...
    public VolunteerStatsVO getPersonalStats(Long userId) {
        ...
        // 4. 最近服务记录（真实数据）
        List<ActivityRegistration> recentRegs = registrationMapper.selectList(
                new LambdaQueryWrapper<ActivityRegistration>()
                        .eq(ActivityRegistration::getVolunteerId, volunteer.getId())
                        .eq(ActivityRegistration::getIsDeleted, 0)
                        .in(ActivityRegistration::getStatus, ActivityRegistration.STATUS_SIGNED_IN, ActivityRegistration.STATUS_COMPLETED)
                        .orderByDesc(ActivityRegistration::getUpdateTime)
                        .last("LIMIT 10"));

        List<VolunteerStatsVO.RecentRecordItem> recentRecords = new ArrayList<>();
        if (!recentRegs.isEmpty()) {
            List<Long> activityIds = recentRegs.stream()
                    .map(ActivityRegistration::getActivityId)
                    .distinct()
                    .collect(Collectors.toList());
            Map<Long, com.volunteer.entity.Activity> activityMap = activityMapper.selectBatchIds(activityIds).stream()
                    .collect(Collectors.toMap(com.volunteer.entity.Activity::getId, a -> a, (a, b) -> a));

            DateTimeFormatter dateFmt = DateTimeFormatter.ofPattern("yyyy-MM-dd");
            for (ActivityRegistration reg : recentRegs) {
                VolunteerStatsVO.RecentRecordItem item = new VolunteerStatsVO.RecentRecordItem();
                item.setId(reg.getId());
                com.volunteer.entity.Activity activity = activityMap.get(reg.getActivityId());
                item.setActivityTitle(activity != null ? activity.getTitle() : ("活动#" + reg.getActivityId()));

                LocalDateTime dt = reg.getSignOutTime() != null ? reg.getSignOutTime()
                        : (reg.getSignInTime() != null ? reg.getSignInTime()
                                : (reg.getUpdateTime() != null ? reg.getUpdateTime() : reg.getCreateTime()));
                item.setDate(dt != null ? dt.format(dateFmt) : "");
                item.setHours(reg.getActualHours() == null ? "0" : reg.getActualHours().stripTrailingZeros().toPlainString());
                item.setPoints(reg.getActualPoints() == null ? 0 : reg.getActualPoints());
