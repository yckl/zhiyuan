...
public class ActivityController {
    ...
    /**
     * 兼容旧版前端分页接口：/activity/page?pageNum=1&pageSize=10
     */
    @GetMapping("/page")
    @Operation(summary = "活动列表(兼容路由)", description = "兼容旧版前端的page/pageSize参数")
    public Result<Page<ActivityDTO>> pageActivitiesCompat(
        ...
        return listActivities(query);
    }

    /**
     * 兼容旧版前端推荐接口：/activity/recommend
     */
    @GetMapping("/recommend")
    @Operation(summary = "推荐活动(兼容路由)", description = "兼容旧版前端推荐列表接口")
    public Result<Page<ActivityDTO>> recommendActivitiesCompat(
